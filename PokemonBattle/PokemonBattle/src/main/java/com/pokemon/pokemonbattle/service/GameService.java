...
public class GameService {
    ...
    // ===== USE ITEM =====
// ===== USE ITEM WITH PROPER ITEM LOGIC =====
    public String useItem(int itemIndex) {
        // 1. Cek status pertempuran aktif menggunakan properti asli kelas
        if (!battleActive) {
            return "Tidak ada pertempuran aktif!";
        }

        // 2. Ambil objek Pokemon aktif langsung dari objek currentPlayer
        if (currentPlayer == null || currentPlayer.getActivePokemon() == null) {
            return "Tidak ada pokemon aktif di arena!";
        }

        Pokemon activePokemon = currentPlayer.getActivePokemon();

        int currentHp = activePokemon.getHp();
        int maxHp = activePokemon.getMaxHp();
        int healAmount = 0;
        String itemLabel = "";

        // 3. Terapkan logika penambahan darah berdasarkan indeks item
        switch (itemIndex) {
            case 0: // Potion
                healAmount = 30;
                activePokemon.setHp(Math.min(currentHp + healAmount, maxHp));
                itemLabel = "Potion";
                break;

            case 1: // Antidote
                // Fitur opsional status efek racun
                itemLabel = "Antidote";
                break;

            case 2: // Full Restore
                healAmount = maxHp - currentHp;
                activePokemon.setHp(maxHp);
                itemLabel = "Full Restore";
                break;

            case 3: // Full Revive
                if (currentHp <= 0) {
                    activePokemon.setHp(maxHp);
                    itemLabel = "Full Revive";
                } else {
                    return activePokemon.getName() + " masih segar bugar, tidak butuh Revive!";
                }
                break;
        ...
        // Kembalikan pesan log dinamis yang rapi untuk dikonsumsi frontend
        if (itemIndex == 2 || itemIndex == 3) {
            return "Gunakan item! HP " + activePokemon.getName() + " pulih sepenuhnya menggunakan " + itemLabel + "!";
        }
        return "Gunakan item! HP " + activePokemon.getName() + " pulih " + healAmount + " menggunakan " + itemLabel;
    }
    ...
    public static class BattleState {
        ...
        public void setCaptureAttempts(int a) {
            this.captureAttempts = a;
        }
        ...
        public void setCanCapture(boolean c) {
            this.canCapture = c;
        }
    }
